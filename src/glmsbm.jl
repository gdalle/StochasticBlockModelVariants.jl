...
$(TYPEDFIELDS)
"""
@kwdef struct LatentsGLMSBM{R<:Real} <: AbstractLatents
...
discrete_values(latents::LatentsGLMSBM) = latents.s
continuous_values(latents::LatentsGLMSBM) = latents.w
...
$(TYPEDFIELDS)
"""
@kwdef struct ObservationsGLMSBM{R<:Real,G<:AbstractGraph{Int}} <: AbstractObservations
...
function Random.rand!(rng::AbstractRNG, F::Matrix, glmsbm::GLMSBM)
    (; N, M, Pʷ) = glmsbm
...
function Base.rand(rng::AbstractRNG, ::RademacherWeightPrior{R}, M::Integer) where {R}
    return rand(rng, (-one(R), +one(R)), M)
end

function Base.rand(rng::AbstractRNG, ::GaussianWeightPrior{R}, M::Integer) where {R}
    return randn(rng, R, M)
end
